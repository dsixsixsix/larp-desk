...
export async function createShapesForAssets(
	...
	for (let i = 0; i < assets.length; i++) {
		...
		if (partial) {
			...
			// Step right by whatever this shape occupies, so dropping several files lays them out
			// in a row. An asset type that carries no intrinsic width (a document card, say) still
			// produces a sized shape, so fall back to the partial's own width rather than leaving
			// the point where it was and stacking every shape on top of the first.
			currentPoint.x += getAssetShapeWidth(asset, partial)
		}
	}
...
function getAssetShapeWidth(asset: TLAsset, partial: TLShapePartial): number {
	if ('w' in asset.props && typeof asset.props.w === 'number') return asset.props.w
	const props = partial.props as { w?: unknown } | undefined
	return typeof props?.w === 'number' ? props.w : 0
}
