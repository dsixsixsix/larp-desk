...
test('closing the client flushes changes made inside the persist throttle window', async () => {
	const { client, tick } = testClient()
	await tick()
	// The first write is a full snapshot; get it out of the way so the flush below is a diff.
	client.store.put([PageRecordType.create({ name: 'test', index: 'a0' as IndexKey })])
	await tick()
	expect(client.db.storeChanges).not.toHaveBeenCalled()

	// No tick: this change is still sitting in the throttled diff queue when the client closes,
	// which is what happens when the editor unmounts to switch documents.
	client.store.put([PageRecordType.create({ name: 'test2', index: 'a1' as IndexKey })])
	client.close()
	await tick()
...
test('closing the client flushes the store history first', async () => {
	// Store listeners fire on the next frame, so without this the diff queue is still empty when
	// the close-time flush looks at it.
	const { client, tick } = testClient()
	await tick()
	const flushHistory = vi.spyOn(client.store, '_flushHistory')
	client.close()
	expect(flushHistory).toHaveBeenCalled()
})

test('closing the client waits for an in-flight write before closing the database', async () => {
	const idbOperationResult = promiseWithResolve<void>()
	const { client, tick } = testClient()
	client.db.storeSnapshot.mockImplementationOnce(() => idbOperationResult)
	const close = vi.spyOn(client.db, 'close')
