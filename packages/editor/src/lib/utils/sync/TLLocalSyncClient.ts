...
export class TLLocalSyncClient {
	...
	close() {
		this.debug('closing')
		// Store listeners fire on the next frame, so a change made in the same tick as the close —
		// exactly what the flush below exists to save — hasn't reached the diff queue yet, and
		// never will once the subscription is disposed.
		this.store._flushHistory()
		// The store subscription, the broadcast channel and the unload listeners all go now, so no
		// further changes can arrive. The database has to outlive them for the final flush below.
	...
	/**
	 * Persists are throttled, so closing inside that window loses the last edits — which is every
	 * close that isn't a page unload: unmounting the editor to switch documents, say. `didDispose`
	 * stops a persist from running, so it's only set once the last write has landed.
	 */
	private async flushAndDispose() {
		try {
			if (this.didLoad) {
				// A write already in flight has to finish before its own queue can be re-checked;
				// anything that arrived while it ran then needs one more pass.
				if (this.persistPromise) await this.persistPromise
				this.persistIfNeeded()
				if (this.persistPromise) await this.persistPromise
			}
		} catch {
			// doPersist reports write failures itself.
		}
	...
	/** The write in flight, if any. `close` waits on it before dropping the db connection. */
