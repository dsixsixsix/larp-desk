...
export const adminRoutes = createRouter<Environment>()
	...
	// Runs a storage GC pass now instead of waiting for the cron. `trashRetentionDays` and
	// `assetRetentionDays` override the configured grace periods: 0 purges everything already in
	// the trash, which is how the backlog that predates migration 050 gets reclaimed (that
	// migration dates the whole backlog from its own deploy, so the scheduled pass would
	// otherwise sit on it for a full retention period).
	.post('/app/admin/storage_gc', async (res, env) => {
		const url = new URL(res.url)
		const parseDays = (param: string) => {
			const raw = url.searchParams.get(param)
			if (raw === null) return undefined
			const days = Number(raw)
			assert(Number.isFinite(days) && days >= 0, `${param} must be a non-negative number`)
			return days
		}
		const warnings: string[] = []
		const result = await runScheduledStorageGc(env, {
			trashRetentionDays: parseDays('trashRetentionDays'),
			assetRetentionDays: parseDays('assetRetentionDays'),
			onError: (error, context) => {
				warnings.push(`${JSON.stringify(context)}: ${error}`)
			},
		})
