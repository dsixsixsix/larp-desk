...
/**
 * `commonSecurityHeaders` as a Caddy `header` block, for the self-hosted build.
 *
 * Written into the built directory rather than committed, so the Caddyfile imports a file that is
 * regenerated on every build and cannot fall behind `csp.ts`.
 */
function caddyHeaderSnippet() {
	const lines = Object.entries(commonSecurityHeaders)
		// Caddy takes a header value as one token, and every one of these contains spaces.
		.map(([name, value]) => `\t${name} ${JSON.stringify(value)}`)
		.join('\n')
	return `# Generated by apps/dotcom/client/scripts/build.ts. Do not edit.\nheader {\n${lines}\n}\n`
}
...
/**
 * Builds a plain `dist` for a reverse proxy to serve, instead of a Vercel deployment.
 *
 * The Vercel path below encodes routing, caching and security headers in `config.json`, which is
 * Vercel's alone to read. A self-hosted deployment gets the same three things from its proxy, so
 * this mode skips that file and emits a Caddy snippet for the headers — generated from the same
 * `csp` constant, so the policy cannot drift between the two ways of shipping the app.
 *
 * The SPA route list is skipped with it: it exists to turn react-router's patterns into Vercel
 * rewrites, and a proxy's catch-all to `index.html` covers the same ground without the snapshot.
 */
const SELF_HOSTED = process.env.SELF_HOSTED === '1'

async function build() {
	// make sure we have the latest routes
	if (!SELF_HOSTED) await exec('yarn', ['test', 'src/routes.test.tsx'])
	const spaRoutes = SELF_HOSTED ? [] : loadSpaRoutes()
	await exec('../../../node_modules/.bin/vite', ['build', '--emptyOutDir'])
	// Injecting debug ids is only worth the step when something will upload the maps afterwards.
	if (!SELF_HOSTED || process.env.SENTRY_AUTH_TOKEN) {
	...
	// Everything below reads the directory that will actually be served, whichever mode built it.
	const outDir = SELF_HOSTED ? 'dist' : '.vercel/output/static'
	...
	const sourceMaps = readdirSync(join(outDir, 'assets')).filter((f) => f.endsWith('.map'))
	for (const map of sourceMaps) {
		rmSync(join(outDir, 'assets', map))
	}

	// Add fonts to preload into index.html
	const assetsList = readdirSync(join(outDir, 'assets'))
	...
	writeFileSync(join(outDir, 'index.html'), newIndex)

	if (SELF_HOSTED) {
		// One source of truth for the policy: the proxy imports this, the Vercel path below inlines
		// the same constant, so a change to csp.ts reaches both without being retyped.
		writeFileSync(join(outDir, 'security-headers.caddy'), caddyHeaderSnippet())
		await reportBundleSize(outDir)
		return
	}
