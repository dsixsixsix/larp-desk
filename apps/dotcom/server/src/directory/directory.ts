...
/**
 * Failed admin sign-ins allowed before the service stops checking, and for how long.
 *
 * The admin secret is the only thing standing between a visitor and the ability to create, delete
 * and un-invite everything. Cloudflare's Durable Object serialized its requests, so a lockout there
 * really did bound the guess rate; a single Node process gives the same guarantee, since every
 * check runs on the one event loop that holds this counter. Held in memory rather than storage on
 * purpose: a restart resets the counter, but restarting is not something a live guessing run can
 * cause.
 */
 ...
 *
 * There is exactly one of these, as there was exactly one Durable Object before it, because every
 * question it answers ("is this person allowed on this board", "who is in this workspace") spans
 * all of the data at once, and sharding it by anything would turn each of those into a fan-out.
 *
 * Board *contents* never reach it: those stay in each browser's IndexedDB. What is stored here is
 * only the directory around them.
 */
export class UnoDirectoryService {
	private adminLoginFailures = 0
	private adminLockedUntil = 0

	constructor(
		private readonly sql: UnoDirectorySql,
	...
	async deleteBoard(sessionToken: string, boardId: string): Promise<UnoDirectory | null> {
		...
		this.presence.evictAll([boardId])
		return getDirectoryForUser(this.sql, admin)
	}
	...
	async deleteWorkspace(sessionToken: string, workspaceId: string): Promise<UnoDirectory | null> {
		...
		this.presence.evictAll(boardIds)
		return getDirectoryForUser(this.sql, admin)
	}
	...
	async removeMember(
		...
		this.presence.evictUser(lostBoardIds, userId)
		return listMembers(this.sql)
	}
