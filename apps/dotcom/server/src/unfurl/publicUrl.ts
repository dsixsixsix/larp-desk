/**
 * The check that keeps a URL the user chose from aiming this server's `fetch` somewhere the user
 * could not reach themselves.
 *
 * {@link parsePublicHttpUrl} refuses the forms that need no resolution to be dangerous: an address
 * literal in a range that belongs to somebody's network, and the names that mean "this machine" or
 * "this LAN" everywhere. On Cloudflare that was the whole of the check, because a worker has no
 * resolver and a name pointed at a private address still got through.
 *
 * Here there is a resolver, so {@link assertPublicHttpUrl} follows the name to the addresses it
 * answers with and applies the same ranges to those. That closes the case the Cloudflare version
 * documented as open — `internal.example.com` resolving to `10.0.0.5`. What neither can close is
 * the address changing between the check and the fetch, which is why callers still pair this with
 * a rate limit rather than relying on it alone.
 */
...
/** Whether a resolved address literal names somebody's network rather than the public internet. */
export function isBlockedAddress(address: string): boolean {
	if (address.includes(':')) return isBlockedIpv6(address)
	const packed = parseIpv4(address)
	return packed === null ? true : isBlockedIpv4(packed)
}

/**
 * The URL as a `URL`, having also checked where its hostname actually points.
 *
 * Returns null for anything {@link parsePublicHttpUrl} refuses, and for a name that resolves to an
 * address inside a private range — the case a resolver-less runtime cannot see. A name that does
 * not resolve at all is refused too: there is nothing to fetch, and failing here is cheaper than
 * failing in the fetch.
 */
export async function assertPublicHttpUrl(raw: string): Promise<URL | null> {
	const url = parsePublicHttpUrl(raw)
	if (!url) return null

	const hostname = url.hostname.toLowerCase().replace(/\.$/, '')
	// An address literal was already checked against the ranges; resolving it would only hand the
	// same value back.
	if (hostname.startsWith('[') || parseIpv4(hostname) !== null) return url
	...
	// Every answer must be public. A name that returns one public and one private address is the
	// shape of the attack, not a misconfiguration to be lenient about.
